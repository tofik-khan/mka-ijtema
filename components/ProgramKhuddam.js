import React from "react";

import Container from "react-bootstrap/Container";
import Row from "react-bootstrap/Row";
import Col from "react-bootstrap/Col";

export default function ProgramKhuddam() {
  return (
    <section>
      <Container className="my-5">
        <Row className="text-center">
          <Col>
            <h2>Friday August 19th 2022</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md={8}>
            <table className="table table-hover text-center">
              <thead>
                <tr>
                  <th className="col-sm-3 col-md-2">Time</th>
                  <th>Activity</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>5:15 AM</td>
                  <td>Salatul Fajar</td>
                </tr>
                <tr>
                  <td>5:30 AM</td>
                  <td>Sleep Catchup + Getting Ready</td>
                </tr>
                <tr>
                  <td>8:00 AM</td>
                  <td>Breakfast</td>
                </tr>
                <tr>
                  <td>10:00 AM</td>
                  <td>Shura &amp; Ijtema Registration</td>
                </tr>
                <tr>
                  <td>12:00 PM</td>
                  <td>Lunch</td>
                </tr>
                <tr>
                  <td>1:15 PM</td>
                  <td>
                    Jummah Prayers
                    <br />
                    (First Azan is 1:00PM)
                  </td>
                </tr>
                <tr>
                  <td>2:15 PM</td>
                  <td>
                    <Container>
                      <Row className="py-1">
                        <Col>
                          <strong>Service Activity</strong>
                          <br />
                          Packing of 50k meal kits (BRM Basement)
                          <br />
                          <a
                            href="http://tinyurl.com/meals4needy"
                            target="_blank"
                            rel="noreferrer"
                          >
                            Meals4Needy
                          </a>
                        </Col>
                      </Row>
                      <hr />
                      <Row>
                        <Col>
                          <strong>Taleem Qualifiers</strong>
                          <ul>
                            <li>Tilawat</li>
                            <li>Nazam</li>
                            <li>Azan</li>
                            <li>Prepared Speech</li>
                            <li>English Poem</li>
                            <li>Impromptu Speech</li>
                          </ul>
                          <div>* Pre-approval required to compete</div>
                        </Col>

                        <Col>
                          <strong>HUB</strong>
                          <ul>
                            <li>Letter Writing Center</li>
                            <li>MKA Fit Challenge</li>
                            <li>Bull Riding</li>
                            <li>Ask an Imam</li>
                            <li>MKA Bazaar</li>
                          </ul>
                        </Col>
                      </Row>
                    </Container>
                  </td>
                </tr>
                <tr>
                  <td>
                    <strong>6:00 PM</strong>
                  </td>
                  <td>
                    <strong>Khuddam Ijtema Opening Session</strong>
                  </td>
                </tr>
                <tr>
                  <td>7:00 PM</td>
                  <td>Team Taleem Competitions</td>
                </tr>
                <tr>
                  <td>7:30 PM</td>
                  <td>Dinner</td>
                </tr>
                <tr>
                  <td>9:00 PM</td>
                  <td>Salatul Maghrib &amp; Isha</td>
                </tr>
                <tr>
                  <td>9:30 PM</td>
                  <td>
                    <strong>The Existence Project:</strong>
                    <br />
                    Murrabi Sabahat Ali Sahib
                  </td>
                </tr>
                <tr>
                  <td>10:45 PM</td>
                  <td>Prep for Bed &amp; Lights Off</td>
                </tr>
              </tbody>
            </table>
          </Col>
        </Row>
      </Container>
      <Container className="my-5">
        <Row className="text-center">
          <Col>
            <h2>Saturday August 20th 2022</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md={8}>
            <table className="table table-hover text-center">
              <thead>
                <tr>
                  <th className="col-sm-3 col-md-2">Time</th>
                  <th>Activity</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>4:15 AM</td>
                  <td>Tahajjud</td>
                </tr>
                <tr>
                  <td>5:15 AM</td>
                  <td>Salatul Fajr &amp; Dars</td>
                </tr>
                <tr>
                  <td>5:30 AM</td>
                  <td>Sleep Catch Up + Getting Ready</td>
                </tr>
                <tr>
                  <td>7:00 AM</td>
                  <td>Breakfast</td>
                </tr>
                <tr>
                  <td>8:30 AM</td>
                  <td>Taleem Session Preparation &amp; Location Assignment</td>
                </tr>
                <tr>
                  <td>9:00 AM</td>
                  <td>
                    <Container>
                      <Row>
                        <Col>
                          <strong>Taleem Competitions</strong>
                          <ul>
                            <li>Tilawat</li>
                            <li>Nazam</li>
                            <li>Azan</li>
                            <li>Prepared Speech</li>
                            <li>Impromptu Speech</li>
                          </ul>
                        </Col>
                        <Col>
                          <strong>Workshops</strong>
                          <ul>
                            <li>ROR: TEP Exhibit</li>
                            <li>SOT: Public Speaking Seminar</li>
                            <li>Amoomi Team: Career in Law Enforcement</li>
                          </ul>
                        </Col>
                        <Col>
                          <strong>MKA Hub</strong>
                          <ul>
                            <li>Need for Speed: Car Show</li>
                            <li>English Poem Competition</li>
                            <li>MKA Hangout</li>
                          </ul>
                        </Col>
                      </Row>
                    </Container>
                  </td>
                </tr>
                <tr>
                  <td>11:30 AM</td>
                  <td>
                    MKA TED Talks <br /> Securing the Bag ... In the Hereafter{" "}
                    <br /> Ashfaq Khan, Muavin Sadr, MKA USA
                  </td>
                </tr>
                <tr>
                  <td>12:30 PM</td>
                  <td>Lunch</td>
                </tr>
                <tr>
                  <td>1:30 PM</td>
                  <td>Salatul Zuhur &amp; Asr</td>
                </tr>
                <tr>
                  <td>1:45 PM</td>
                  <td>Waqf-e-Nau Department Presentation</td>
                </tr>
                <tr>
                  <td>2:00 PM</td>
                  <td>
                    MKA TED Talks <br /> Finding God through Marriage <br />{" "}
                    Rehman Nasir, Additional Mohtamim Tarbiyyat
                  </td>
                </tr>
                <tr>
                  <td>3:00 PM</td>
                  <td>
                    <Container>
                      <Row>
                        <Col md={4} xs={12}>
                          <strong>
                            Taleem Competitions <br /> (Open to All)
                          </strong>
                          <ul>
                            <li>Hifz-e-Quran</li>
                            <li>Hifz-e-Ahadith</li>
                            <li>Hifz-e-Adi-yyah</li>
                            <li>Urdu Speech</li>
                          </ul>
                        </Col>
                        <Col md={4} xs={12}>
                          <strong>Sports Competitions</strong>
                          <ul>
                            <li>Basketball</li>
                            <li>Soccer</li>
                            <li>Volleyball</li>
                            <li>Cricket</li>
                            <li>Obstacle Course</li>
                          </ul>
                        </Col>
                        <Col md={4} xs={12}>
                          <strong>MKA Hub</strong>
                          <ul>
                            <li>MKA Cafe</li>
                            <li>Basics of Cooking</li>
                            <li>MKA Fit</li>
                            <li>Tabligh Escape Room</li>
                            <li>Exhibition</li>
                            <li>Much more...</li>
                          </ul>
                        </Col>
                      </Row>
                    </Container>
                  </td>
                </tr>
                <tr>
                  <td>5:00 PM</td>
                  <td>Message Relay (By Registration Only)</td>
                </tr>
                <tr>
                  <td>5:30 PM</td>
                  <td>
                    Air Soft and Archery Competition: Jamaat Amila vs Khuddam
                    Amila
                  </td>
                </tr>
                <tr>
                  <td>6:30 PM</td>
                  <td>Kabaddi Exhibition Match</td>
                </tr>
                <tr>
                  <td>7:00 PM</td>
                  <td>Regional BBQ Cook-Off &amp; dinner</td>
                </tr>
                <tr>
                  <td>9:00 PM</td>
                  <td>Salatul Maghrib &amp; Isha</td>
                </tr>
                <tr>
                  <td>9:15 PM</td>
                  <td>Special Session with Respected Ameer Sahib USA</td>
                </tr>
                <tr>
                  <td>10:45 PM</td>
                  <td>Prep for Bed &amp; Lights Off</td>
                </tr>
              </tbody>
            </table>
          </Col>
        </Row>
      </Container>
      <Container className="my-5">
        <Row className="text-center">
          <Col>
            <h2>Sunday August 21th 2022</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md={8}>
            <table className="table table-hover text-center">
              <thead>
                <tr>
                  <th className="col-sm-3 col-md-2">Time</th>
                  <th>Activity</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>4:15 AM</td>
                  <td>Tahajjud</td>
                </tr>
                <tr>
                  <td>5:15 AM</td>
                  <td>Salatul Fajr &amp; Dars</td>
                </tr>
                <tr>
                  <td>5:30 AM</td>
                  <td>Sleep Catch up</td>
                </tr>
                <tr>
                  <td>7:00 AM</td>
                  <td>Breakfast</td>
                </tr>
                <tr>
                  <td>8:00 AM</td>
                  <td>Breakfast + Waqar-e-Amal</td>
                </tr>
                <tr>
                  <td>9:00 AM</td>
                  <td>
                    <strong>Taleem Competition Finals</strong>
                    <br />* Audience voting will count toward the total score of
                    each participant.
                  </td>
                </tr>
                <tr>
                  <td>10:00 AM</td>
                  <td>Mufti Muhammad Sadiq Award Ceremony</td>
                </tr>
                <tr>
                  <td>11:00 AM</td>
                  <td>
                    <strong>
                      Live Stream: Concluding Address of Jalsa Salana Germany
                      <br />
                      Hazrat Khalifatul Masih V (aba)
                    </strong>
                  </td>
                </tr>
                <tr>
                  <td>11:30 AM</td>
                  <td>
                    <strong>The Tipping Point</strong>
                    <br />
                    Tabligh Session with Maulana Azhar Haneef Sahib
                    <br />
                    (Naib Ameer and Missionary In-Charge)
                  </td>
                </tr>
                <tr>
                  <td>12:00 PM</td>
                  <td>Closing Session &amp; Award Ceremony</td>
                </tr>
                <tr>
                  <td>1:30 PM</td>
                  <td>Salatul Zuhr &amp; Asr</td>
                </tr>
                <tr>
                  <td>2:00 PM</td>
                  <td>To-Go Lunch</td>
                </tr>
                <tr>
                  <td>2:30 PM</td>
                  <td>Wrap Up &amp; Departure</td>
                </tr>
              </tbody>
            </table>
          </Col>
        </Row>
      </Container>
    </section>
  );
}
